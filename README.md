# TaskSheet

TaskSheet turns resident care assignments, unit responsibilities, and important shift information into organized, print-ready HCA and LPN working sheets.

## Release status

**TaskSheet 1.0.0-rc.21 — Pilot Artifact Ready**

The operational feature set is frozen. The tagged Windows x64 pilot artifact is ready for clean-machine validation. Production approval still requires a completed controlled pilot with zero open P0/P1 defects and a successful clean-Windows validation.

Start with the [release closure index](docs/release/README.md) for pilot execution, validation checklists, documentation, and release gates.

## Local verification

```powershell
npm test
npm run build
```

The current build status and toolchain are recorded in [Version and Build Information](docs/release/VERSION_BUILD_INFO.md).
