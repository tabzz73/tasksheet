...
import { CalendarClock, CheckCircle2, ClipboardCheck, ExternalLink, RotateCcw, ShieldAlert, XCircle } from 'lucide-react';
...
export const FollowUpActionsModal: React.FC<FollowUpActionsModalProps> = ({ entry, today, onClose, onOpenResident, onChanged }) => {
  const [confirmRequest, setConfirmRequest] = useState<ConfirmDialogRequest | null>(null);
  const [extendDate, setExtendDate] = useState('');

  if (!entry) return null;
  const { task, resident } = entry;
      ...
      <Modal isOpen={!!entry} onClose={onClose} title="Follow-up Actions" subtitle={`${resident.roomNumber} · ${resident.firstName} ${resident.lastName}`} maxWidth="sm">
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            {isOccurrenceMode ? (
              <>
                <ActionButton icon={ClipboardCheck} tone="positive" label="Record Next Collection" onClick={() => applyAndStay(() => followUpActions.recordOccurrence(task.id))} />
...
function describeCurrentState(task: ResidentTask, today: string): string {
  ...
  if (tracking?.requiredOccurrences) {
    return formatOccurrenceProgressLabel(tracking.completedOccurrences || 0, tracking.requiredOccurrences);
  }
  if (tracking) {
