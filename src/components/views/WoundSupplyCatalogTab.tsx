    ...
    <section className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-slate-100 max-h-[520px] overflow-y-auto">
        {filtered.map(product => <div key={product.id} className={`p-3 grid grid-cols-1 lg:grid-cols-[1fr_150px_130px_170px] gap-3 items-center ${!product.isActive ? 'opacity-55' : ''}`}>
          ...
          <div className="flex justify-end gap-1.5"><button type="button" onClick={() => openNew(product)} className="p-2 border border-slate-300 rounded-md" title="Add another size" aria-label={`Add another size for ${product.productName}`}><PackagePlus className="w-3.5 h-3.5" /></button><button type="button" onClick={() => openEdit(product)} className="p-2 border border-slate-300 rounded-md" title="Edit product" aria-label={`Edit ${product.productName}`}><Edit2 className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { db.updateWoundSupplyProduct(product.id, { isActive: !product.isActive, localFormularyStatus: product.isActive ? 'inactive' : 'not_stocked' }); refresh(); }} className={`px-2.5 py-1.5 rounded-md text-[10px] font-bold ${product.isActive ? 'border border-rose-300 text-rose-700' : 'bg-emerald-700 text-white'}`}>{product.isActive ? 'Deactivate' : 'Reactivate'}</button></div>
        </div>)}
      </div>
    </section>
