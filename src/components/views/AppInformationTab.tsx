...
export const AppInformationTab: React.FC<AppInformationTabProps> = ({
  ...
  <div className="space-y-5">
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="bg-gradient-to-r from-[#081D3A] to-[#0D3651] p-5 text-white">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
      ...
      <label className="flex cursor-pointer select-none items-center gap-2.5 text-xs font-semibold text-slate-700">
        ...
        />
        <span>Show the six “Why TaskSheet” overview cards</span>
      </label>
      ...
      {onNavigateToWelcome && (
        <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
          ...
          <button type="button" onClick={() => onNavigateToWelcome(true)} className="inline-flex items-center gap-2 rounded-lg bg-[#081D3A] px-4 py-2 text-xs font-bold text-white hover:bg-[#0D2A54]">
