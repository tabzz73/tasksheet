...
function validateScheduledTimesAssignment(state: AppDatabaseState, candidate: TaskAssignmentCandidate): ValidationResult {
  ...
  const candidateTimesKey = [...times].sort().join(',');
  const duplicate = collection.find(task => task.id !== candidate.id && task.isActive !== false && ('residentId' in task ? task.residentId === candidate.residentId : true) && task.title.trim().toLowerCase() === candidate.title.trim().toLowerCase() && [...('trackingConfig' in task ? task.trackingConfig?.scheduledTimes || [] : [])].sort().join(',') === candidateTimesKey);
  if (duplicate) return { status: 'BLOCKED', code: 'DUPLICATE_TASK', title: 'Duplicate Task Assignment', message: `“${candidate.title}” is already scheduled with these exact times for this resident. Edit the existing assignment instead of creating another copy.`, affectedRecords: [{ id: duplicate.id, type: candidate.kind === 'unit_task' ? 'unit_task' : 'resident_task', label: duplicate.title, time: duplicate.time }], recommendedActions: [{ id: 'edit_existing', label: 'Edit Existing Assignment', kind: 'primary' }, { id: 'cancel', label: 'Cancel', kind: 'cancel' }] };
  return validResult();
}
