...
export const REPORT_FIELDS: Record<ReportDataSource, ReportFieldDefinition[]> = {
  ...
  rooms: [field('room', 'Room / Bed', 'approved_identity', true), field('physicalRoom', 'Physical Room', 'configuration'), field('position', 'Position', 'configuration'), field('wing', 'Wing', 'configuration'), field('floor', 'Floor', 'configuration'), field('resident', 'Resident', 'approved_identity', true), field('status', 'Resident Status', 'operational', true), field('availability', 'Availability', 'operational', true)],
...
export const SYSTEM_REPORT_PRESETS: ReportDefinition[] = [
  ...
  preset('room-directory', 'Room / Occupancy Directory', 'Facility / Setup', 'rooms', ['room', 'physicalRoom', 'position', 'wing', 'floor', 'availability'], 'Configured rooms and occupancy positions.', [], '', [{ field: 'room', direction: 'asc', naturalRoom: true }]),
  preset('available-rooms', 'Available Rooms / Occupancy Positions', 'Facility / Setup', 'rooms', ['room', 'wing', 'floor', 'availability'], 'Active room and bed positions currently available for assignment.', [{ field: 'available', operator: 'is_true' }], '', [{ field: 'room', direction: 'asc', naturalRoom: true }]),
...
function baseRows(source: ReportDataSource, dateRange?: { start: string; end: string }): ReportRow[] {
  ...
  if (source === 'rooms') return state.occupancyPositions.map(position => {
    ...
    return { id: position.id, values: { room: position.displayLabel, physicalRoom: room?.physicalRoomLabel || position.displayLabel, position: position.positionLabel || '—', wing: room?.wing || '', floor: room?.floor || '', resident: residentName(resident), status: resident ? displayStatus(resident.status) : '—', availability: position.active === false ? 'Inactive' : available ? 'Available' : 'Occupied', available } };
  });
