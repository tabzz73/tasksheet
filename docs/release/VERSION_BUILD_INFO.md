...
RC10 preserves RC9 operational and print behavior while modernizing Settings navigation and Facility Setup entry. Settings uses grouped vertical navigation on desktop and a responsive selector on narrow screens; Canadian contact and address fields now provide formatting, validation, and suggestions. Final RC10 verification is recorded in the generated build evidence:
...
The authoritative source is maintained in Git and tagged `v1.0.0-rc.10`. The guarded NSIS pipeline requires a clean tagged `HEAD`, reruns every release gate, builds the Windows x64 installer, and writes the exact commit SHA and installer checksum to the external build-evidence record beside the artifact.

Build output: `dist/`  
Build ID: See generated RC10 build evidence
Build date: See generated RC10 build evidence
Source identifier: See `release/TaskSheet-Build-Evidence-1.0.0-rc.10.txt`
Git tag: `v1.0.0-rc.10`
Pilot installer: `TaskSheet-Setup-1.0.0-rc.10-x64.exe`
