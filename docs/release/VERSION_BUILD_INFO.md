...
RC9 preserves RC8 operational and print behavior and corrects the Residents list three-dot action popup. The menu is rendered outside the clipped list container, chooses an available viewport direction, and remains accessible at window boundaries. Final RC9 verification is recorded in the generated build evidence:
...
The authoritative source is maintained in Git and tagged `v1.0.0-rc.9`. The guarded NSIS pipeline requires a clean tagged `HEAD`, reruns every release gate, builds the Windows x64 installer, and writes the exact commit SHA and installer checksum to the external build-evidence record beside the artifact.

Build output: `dist/`  
Build ID: See generated RC9 build evidence
Build date: See generated RC9 build evidence
Source identifier: See `release/TaskSheet-Build-Evidence-1.0.0-rc.9.txt`
Git tag: `v1.0.0-rc.9`
Pilot installer: `TaskSheet-Setup-1.0.0-rc.9-x64.exe`
