...
RC3 makes fresh installation production-first and keeps demo content as an explicit Settings action. Final RC3 verification is recorded in the generated build evidence:
...
The authoritative source is maintained in Git and tagged `v1.0.0-rc.3`. The guarded NSIS pipeline requires a clean tagged `HEAD`, reruns every release gate, builds the Windows x64 installer, and writes the exact commit SHA and installer checksum to the external build-evidence record beside the artifact.

Build output: `dist/`  
Build ID: See generated RC3 build evidence
Build date: See generated RC3 build evidence
Source identifier: See `release/TaskSheet-Build-Evidence-1.0.0-rc.3.txt`
Git tag: `v1.0.0-rc.3`
Pilot installer: `TaskSheet-Setup-1.0.0-rc.3-x64.exe`
