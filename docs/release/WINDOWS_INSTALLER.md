...
- Filename: `TaskSheet-Setup-1.0.0-rc.6-x64.exe`
...
3. `HEAD` is tagged exactly `v1.0.0-rc.6`.

On success it writes the installer and `TaskSheet-Build-Evidence-1.0.0-rc.6.txt` under the ignored `release/` directory. The evidence record contains commit SHA, tag, clean-tree status, UTC build time, Node/npm versions, command results, installer filename/checksum, and hashes for every `dist/` file.
